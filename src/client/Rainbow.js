...
export default class MenuItem extends Component {
	...
	componentDidMount() {
		...
		for(let i = 0; i < canvas.width; i+= WIDTH){
			columns.push(new Column(this.newRGB(),i));
		}
		for(let i = 0; i < canvas.width; i+= WIDTH){
			columns2.push(new Column(this.newRGB(),i));
		}
		for(let i = 0; i < canvas.height; i+= HEIGHT){
			rows.push(new Row(this.newRGB(),i));
		}
		for(let i = 0; i < canvas.height; i+= HEIGHT){
			rows2.push(new Row(this.newRGB(),i));
		}
		setInterval(()=>{
			ctx.globalAlpha = .25;
			ctx.clearRect(0,0,canvas.width,canvas.height);
			columns.push(new Column(this.newRGB(),canvas.width));
			columns.forEach(col=>{
				col.x -= WIDTH;
			...
			////////////////////////////////////
			rows.push(new Row(this.newRGB(),canvas.height));
			rows.forEach(row=>{
				row.y -= HEIGHT;
			...
			////////////////////////////////////
			columns2.push(new Column(this.newRGB(),0));
			...
			////////////////////////////////////
			rows2.push(new Row(this.newRGB(),0));
			rows2.forEach(row=>{
				row.y += HEIGHT;
				ctx.fillStyle = row.color;
				ctx.fillRect(0,row.y,canvas.width,row.height);
	...
	newRGB(){
		return `rgb(${Math.floor(Math.random()*(new Date().getMinutes() * (256/60)))},${Math.floor(Math.random()*(new Date().getSeconds() * (256/60)))},${Math.floor(Math.random()*(new Date().getMilliseconds() * (256/1000)))})`;
	}
	render() {
